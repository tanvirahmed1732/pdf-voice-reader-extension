...
- **Web pages too, in place** — on any normal page, select text, click the extension icon, press ▶ Play: reading starts at your selection right on the page (sentence + word highlighting, auto-scroll that follows nested scroll containers, click-any-text-to-jump) and continues until stopped. Controls live in a sidebar on the right of the page: hidden until you click the extension icon, then it stays open (and follows whichever tab is active) until you click the icon again or its ✕ — pause/resume/stop or change voice/speed mid-read without leaving the page. Drag its left edge to make it as narrow (down to ~44px, icons only) or as wide as you like; the controls restack to fit. The header button cycles three hosts: the default in-page sidebar with the page pushed aside so nothing hides behind it → a floating window you can drag anywhere by its header and resize from its left edge, bottom edge or bottom-left corner → Chrome's built-in side panel (toggled natively by the icon, 320px minimum) → back to the default via the button in the panel's header. On PDF and browser tabs (where extensions can't inject) the icon opens Chrome's built-in side panel instead. No selection = reads from the top
...
- **Keyboard** — `Space` play/pause, `←` previous sentence, `→` next sentence — in the PDF reader, on web pages read in place, and while the side panel is focused
